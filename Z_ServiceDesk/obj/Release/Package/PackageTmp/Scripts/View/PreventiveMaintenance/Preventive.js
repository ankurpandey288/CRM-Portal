$(document).ready(function () {
    ...
    $(document).on('click', '.editview', function () {
        ...
        if ($.session.get("per_prev_activity_id_pk") != '' || $.session.get("per_prev_activity_id_pk") != null || $.session.get("per_prev_activity_id_pk") == undefined) {
            $.session.remove("per_prev_activity_id_pk");
            $.session.set("per_prev_activity_id_pk", $(this).attr("name"));
...
function GetPreventiveMaintananceStatusConditionWise(per_prev_activity_id) {
    var parm = {
        "per_prev_activity_id_pk": per_prev_activity_id,
    };
    var josnstr = JSON.stringify(parm);
    $.ajax({
        type: "Post",
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/PreventiveMaintenance/GetPeripheralPreventiveMaintananceStatusConditionWise',
        success: function (data) {
...
//                        bSortable: false,
//                        render: function (p_m_activity_id_pk) {
//                            return '<a class="editview"  name="' + p_m_activity_id_pk + '"> ' + p_m_activity_id_pk + '</a>';  // <button class="btn btn-xs btn-success grid-buttons btnedit" name="' + catogory_id_pk + '" style="margin-right:5px;"><i class="fa fa-edit icon-ser"></i></button>
//                        }
//                    },
//                    //{ data: 'serial_number' },
//                    { data: 'asset_cat_name' },
...
function GetPreventiveMaintainanceActivity() {
    $.ajax({
        type: "Get",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/PreventiveMaintenance/GetPerpheralsPreventiveMaintainanceActivity',
        dataType: "json",
        success: function (data) {
            var table;
            if ($.fn.dataTable.isDataTable('#tblPeripheralPreventiveMaintainanceActivity')) {
                table = $('#tblPeripheralPreventiveMaintainanceActivity').DataTable();
            } else {
                table = $('#tblPeripheralPreventiveMaintainanceActivity').DataTable();
            }
            table.destroy();
            $("#tblPeripheralPreventiveMaintainanceActivity").DataTable({
                data: data,
                paging: true,
                sort: false,
                searching: true,
                ...
                columns: [
                    {
                        data: 'per_prev_activity_id_pk',
                        sWidth: '2px',
                        sClass: "view",
                        bSortable: false,
                        render: function (per_prev_activity_id_pk) {
                            return '<input id="checkbox0" class="cb-element" name="' + per_prev_activity_id_pk + '" type="checkbox">';  // <button class="btn btn-xs btn-success grid-buttons btnedit" name="' + catogory_id_pk + '" style="margin-right:5px;"><i class="fa fa-edit icon-ser"></i></button>
                        }
                    },
                    {
                        data: 'per_prev_activity_id_pk',
                        ...
                        bSortable: false,
                        render: function (per_prev_activity_id_pk) {
                            return '<a class="editview"  href="/PreventiveMaintenance/PeripheralsPreventiveMaintenanceCheckListView"  name="' + per_prev_activity_id_pk + '"> <i class="fa fa-eye" data-toggle="tooltip" title="" style="color:#35adaf !important" data-original-title="Normal priority"></i> &nbsp; ' + per_prev_activity_id_pk + '</a>';  // <button class="btn btn-xs btn-success grid-buttons btnedit" name="' + catogory_id_pk + '" style="margin-right:5px;"><i class="fa fa-edit icon-ser"></i></button>
                        }
                    },
                    { data: 'category_name' },
                    { data: 'assets_tag' },
...
function UpdatePreventiveMaintananceStatus(p_m_activity_id_pk) {
    var parm = {
        "per_prev_activity_id_pk": p_m_activity_id_pk, 
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/PreventiveMaintenance/UpdatePeripheralsPreventiveMaintananceStatus',
        success: function (data) {
            if (data.status_id == 1) {
...
function UpdatePreventiveMaintananceAssignTo(p_m_activity_id_pk) {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/PreventiveMaintenance/UpdatePeripheralsPreventiveMaintananceAssignTo',
        success: function (data) {
            if (data.status_id == 1) {
                CreateSuccess(data.status);
