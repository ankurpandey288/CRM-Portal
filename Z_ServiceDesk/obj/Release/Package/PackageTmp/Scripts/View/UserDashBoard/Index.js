   ...
   // alert(number);
    $("#tags").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: 'http://playmediahouse.com/api/api/KnowledgeBase/AutoSearchKnowledgeBase',
                type: "POST",
                dataType: "json",
                data: { solution_title: request.term },
        ...
        select: function (event, ui) {
            ...
            GetKnowledgeBaseAutoSearch(ui.item.id);
        },
        messages: {
            noResults: "", results: ""
        }
...
function GetServiceRequestDetailsByUserId(created_by) {
    var parm = {
...
// Get Knowledge Base 
//Get Task For Edit
function GetKnowledgeBaseAutoSearch(solution_id) {  
    var parm = {
    ...
    $.ajax({
        type: "Post",
        url: 'http://playmediahouse.com/api/api/KnowledgeBase/GetKnowledgeBaseAutoSearch',   
        contentType: "application/json; charset=utf-8",
        data: josnstr,
        dataType: "json",
        success: function (data) {
            $("#txtKBContent").html(data.contents).text();
            $("#txtKBTitle").html(data.solution_title).text();
        },
        error: function (edata) {
            alert("error while feching record.");
        }
