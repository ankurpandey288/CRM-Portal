$(document).ready(function () {
    GetHostName();
    GetAssetSerialNo();
    $('#cbkSerialAndHostName').click(function () {
        if ($(this).is(':checked')) {
            $("#DivHostName").hide();
            $("#DivSerialNo").show();
        } else {
   ...
   // drawChart();
    ...
    GetAssetComponent($.session.get("asset_id_pk"));
  //  GetTaskDetailsByAssetId($.session.get("asset_id_pk"));
   ...
   // GetCpuRamUsesNew(1);
    ...
    $("#btnUpdate").click(function () {
            if (validateAssetsForEdit() == true) {
                AssetUpdate($.session.get("asset_id_pk"));
            } else {
                return false;
            }
    });
    $("#btnSubmitMappingUpdate").click(function () {
        AssetMappingUpdate();
    });
...
function validateAssetsForEdit() {
    var return_val = true;
...
function GetAssetsHardwareList(Asset_id_pk) { 
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.HostName == null || data.HostName == 'NULL') {
                $("#btnSubmitMappingUpdate").hide();
                $("#btnSubmitMapping").show();
            } else {
                $("#btnSubmitMappingUpdate").show();
                $("#btnSubmitMapping").hide();
            }
...
function GetAssetDetails(asset_id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#txtFloorName").text(data.floor_name);
            $("#txtSectionName").text(data.section_name);
            $("#txtBuilding").text(data.sub_location);
            $("#txtRoomNo").text(data.building_room_no);
...
function GetTicketListsByAssetId(asset_id_pk) {
    var parm = {
        'asset_id_pk': asset_id_pk 
    };
    var josnstr = JSON.stringify(parm);
...
// Map Asset System Hardware Info
function InsAssetComponent() {
    var parm = {
        "asset_id_fk": $.session.get("asset_id_pk"),
        "component_id_fk": $("#ddlComponent option:selected").val(),
        "warranty_end_date": $("#txtWarrantyEndDateComp").val(),
        "supplier_id_fk": $("#ddlVendor option:selected").val(),
        "remarks": $("#txtTandC").val(),
    };
    var josnstr = JSON.stringify(parm);
    $.ajax({
        type: "Post",
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/FixedAssets/InsAssetComponent',
        success: function (data) {
            //  alert("Inserted Successfully");           
            if (data.status_id == 0) {
                successnotify(data.status);
...
//Get Task List By Asset Id
function GetAssetComponent(asset_id) { 
    var parm = {
        'asset_id_fk': asset_id
    };
    var josnstr = JSON.stringify(parm);
    $.ajax({
        type: "Post",
        url: 'http://playmediahouse.com/api/api/FixedAssets/GetAssetComponent',
        contentType: "application/json; charset=utf-8",
        data: josnstr,
        dataType: "json",
        success: function (data) {
            var table;
            if ($.fn.dataTable.isDataTable('#tblAssetComponents')) {
                table = $('#tblAssetComponents').DataTable();
            } else {
                table = $('#tblAssetComponents').DataTable();
            }
            table.destroy();
            $("#tblAssetComponents").DataTable({
                data: data,
                paging: false,
                sort: false,
                searching: false,
                ...
                columns: [
                    {
                        ...
                        sClass: "view",
                        bSortable: false,
                        render: function (asset_comp_id_pk) {
                            return '<input id="checkbox0" class="cb-element checkbox tktcbk" name="' + asset_comp_id_pk + '" type="checkbox">';  // <button class="btn btn-xs btn-success grid-buttons btnedit" name="' + catogory_id_pk + '" style="margin-right:5px;"><i class="fa fa-edit icon-ser"></i></button>
                        }
                    },
                    { data: 'asset_comp_id_pk' },
                    { data: 'serial_number' },
...
// Get Asset Audit List
function GetAssetAuditReport(change_set) {
    var parm = {
    ...
    $.ajax({
        type: "Post",
        url: 'http://playmediahouse.com/api/api/Ticket/GetAssetAuditReport',
        contentType: "application/json; charset=utf-8",
        data: josnstr,
        dataType: "json",
        success: function (data) {
...
function AssetMappingUpdate() { 
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: 'http://playmediahouse.com/api/api/FixedAssets/AssetMappingUpdate', 
        success: function (data) {
            if (data.status_id == 1) {
                successnotify(data.status);
                GetAssetsHardwareList($.session.get("asset_id_pk"));
                ...
                GetAssetSystemSound($.session.get("asset_id_pk"));
                GetAssetSystemPrinter($.session.get("asset_id_pk"));
                $("#modalFixedAssMapping").click();
                $("#clrAssetMap").find("select").val(0).change();
            } else {
                warningnotify(data.status);
            }
        },
...
function GetAssetSerialNo() {
    $.ajax({ 
        ...
        dataType: "json",
        success: function (data) {
            $('#ddlAssetSerialNoForMap').html("").append('<option value="0">Select Serial No</option>');
            $(data).each(function () {
                $('#ddlAssetSerialNoForMap').append('<option value=' + this.asset_id_pk + '>' + this.serial_number + '</option>'); 
            });
        },
        error: function (edata) {
            alert("error while feching record.");
